...
elseif multiPer == 1
    ...
    for t = T0 : T 
        ...
        if nPV ~= 0
            ...
            Scap(idxPV-1) = PVcap*inverterSize*solar(t)/baseMVA; % Smax 
    ...
    % SAVE RESULTS ===========================================    
        ...
        Gug_PgTot(t-T0+1,:) = sum(Pd-(Pcap+Pcurt))*baseMVA + sum(Gug_I2R); % P from grid
...
%         figure(202)
%         plot(1:nBuses, Gug_V([13 15 17],:))%;hold on; plot(Gug_QcInd(26,:),'o')
...
%         % PF
%         figure(206)
%         plot(T0:T, Gug_check_PF(:,[12:13 15:16 18]), '-')
%         ylabel('Power Factor'); xlabel('Time')
%         legend({'HH12','HH13', 'HH15', 'HH16', 'HH18'},'Location','Southwest')
%         title('Inverter power factor')
%         ylim([0.5 1.05]); xlim([0 25])
%         set(gcf,'color','w'); grid on
%         % Line Current
%         figure(207)
%         plot(1:nB, Gug_ITot([13 18],:).*testCase.Ibase, '*');hold on % 312.5 is Ibase
%         plot([1 4 7 10 13 16], Gug_ITot([13 18],[1 4 7 10 13 16]).*testCase.Ibase,'o')
...
%         set(gcf,'color','w'); grid on
        ...
        title('Volt/VAR Inverter reactive Power')
        set(gcf,'color','w'); grid on
        % voltage profile over feeder
        f202 = figure(202)
        movegui(f202,'north');
        ...
        title('Volt/VAR Voltage profile')
        set(gcf,'color','w'); grid on
        % HOusehold power curtailment
        f2040 = figure(2040)
        movegui(f2040,'south');
        ...
        title('Volr/VAR Inverter power curtailment')
        set(gcf,'color','w'); grid on
        % PF
        f206 = figure(206)
        movegui(f206,'northeast');
        ...
        title('Volt/VAR Inverter power factor')
        set(gcf,'color','w'); grid on
        % Line Current
        f207 = figure(207)
        movegui(f207,'southwest');
