...
mpc = IEEE_9BUS_PV; 
%% AC OPF
solarScen = [1 1.5 2.0 2.5]; 
%changePav = 6; % No of Bus with demand change
ACOPF_V = complex(zeros(size(mpc.bus,1),length(solarScen))); % Bus Voltage Vector 
ACOPF_I2R = complex(zeros(size(mpc.bus,1)-1,length(solarScen))); % Line Loss Vector
ACOPF_I = complex(zeros(size(mpc.bus,1)-1,length(solarScen))); % Line Current
for l = 1 : length(solarScen) 
    mpopt = mpoption('model','AC', 'pf.tol', 1e-4,'opf.ac.solver','DEFAULT'); 
    mpc.gen(2,[9,10]) = mpc.gen(2,[9 10])*solarScen(l); 
...
voltageVec = complex(zeros(size(mpc.bus,1),length(solarScen))); % empty vector for voltage scenarios
lossVec = complex(zeros(size(mpc.bus,1)-1,length(solarScen))); % lone loss vector
changePav = mpc.gen(2,1)-1; % between 2-10
solarScen = [1 1.5 2 2.5];
for k = 1 : length(solarScen)
    Pav(changePav) = mpc.gen(2,10)*solarScen(k); 
    Sinj(changePav) = Pav(changePav)*1.1; % inverter capacity vector 
% START ====================================================
...
cvx_begin 
    ...
    Obj1 = 1 * quad_form(Obj1_Vec, Ymn); %Eq.1
    Obj2 = 1 * quad_form(Pc, A) + quad_form(Qc, C) + B' * Pc + D' * abs(Qc); %Eq.2
    Obj3 = 0;
