function [V, Pcurt, Qc, Vmax, Gug_V, Gug_I2R, Gug_ITot, Gug_Vdrop, Gug_Preal, Gug_Sreal, Gug_PgTot, Gug_QgTot, Gug_I2RTot, Gug_PcTot, Gug_QcTot, Gug_max_Scap, Gug_max_Pcap, Gug_Pinj, Gug_check_PF, Gug_PcHH] = Guggilam(testCase, T, T0, solar, loadHH, multiPer, per, plotting, PVcap)
...
elseif multiPer == 1 % multiperiod model
    ...
    % SAVE RESULTS ===========================================
        ...
        % validate results
        Gug_check_Sinj(t-T0+1,:) = sqrt(Qc.^2+(Pcap).^2); % FOR GRAPHS: real inverter capacity considering Pc
    ...
    if plotting == 1
        ...
        figure(100)
        plot(1:nB, min(Gug_Preal([26 30 34],:), Gug_Pinj([26 30 34],:)), 'x'); hold on;
        plot(1:nB, (Gug_max_Pcap([26 30 34],:)),'s')
