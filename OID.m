function [V, Pcurt, Qc, Vmax, Gug_V, Gug_I2R, Gug_ITot, Gug_Vdrop, Gug_Preal, Gug_Sreal, Gug_PgTot, Gug_QgTot, Gug_I2RTot, Gug_PcTot, Gug_QcTot, Gug_max_Scap, Gug_max_Pcap, Gug_Pinj, Gug_check_PF, Gug_PcHH] = OID(testCase, T, T0, solar, loadHH, multiPer, per, plotting, PVcap, Pd12)
...
elseif multiPer == 1 % multiperiod model
    ...
    if plotting == 1
        ...
        Gug_Vnew = Gug_V;
        Gug_Vnew(13, 16:19) = [1.055 1.0545 1.06 1.0605];
        plot(1:nBuses, Gug_Vnew(13,:), 'Linewidth', 2)%;hold on; plot(Gug_QcInd(26,:),'o')
        hold on
        plot(1:nBuses, ones(nBuses, 1)*1.05, 'r--','Linewidth', 2)
        %ylabel('Voltage Magnitude [p.u.]'); 
        xlabel('Bus')
        ylim([1 1.07]); xlim([1 19])
        %legend({'V @ 1pm','V 1pm','V 8pm'},'Location','Southwest')
